 ...
 *
 * To run this cn.spark.example locally, you may run publisher as
 *    `$ bin/run-cn.spark.example \
 *      org.apache.spark.example.streaming.SimpleZeroMQPublisher tcp://127.0.1.1:1234 foo.bar`
 * and run the cn.spark.example as
 *    `$ bin/run-cn.spark.example \
 *      org.apache.spark.example.streaming.ZeroMQWordCount tcp://127.0.1.1:1234 foo`
 */
// scalastyle:on
