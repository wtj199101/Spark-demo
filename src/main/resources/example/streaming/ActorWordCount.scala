 ...
 *
 * To run this cn.spark.example locally, you may run Feeder Actor as
 *    `$ bin/run-cn.spark.example org.apache.spark.example.streaming.FeederActor 127.0.1.1 9999`
 * and then run the cn.spark.example
 *    `$ bin/run-cn.spark.example org.apache.spark.example.streaming.ActorWordCount 127.0.1.1 9999`
 */
object ActorWordCount {
  def main(args: Array[String]) {
     ...
     * should be same.
     *
     * For cn.spark.example: Both actorStream and SampleActorReceiver are parameterized
