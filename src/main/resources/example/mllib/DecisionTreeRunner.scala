...
/**
 * An cn.spark.example runner for decision trees and random forests. Run with
 * {{{
 * ./bin/run-cn.spark.example org.apache.spark.example.mllib.DecisionTreeRunner [options]
 * }}}
...
object DecisionTreeRunner {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("DecisionTreeRunner") {
      head("DecisionTreeRunner: an cn.spark.example decision tree app.")
      opt[String]("algo")
  ...
  private[mllib] def loadDatasets(
    ...
    val (examples, classIndexMap, numClasses) = algo match {
      case Classification => {
        ...
        println(s"Per-class cn.spark.example fractions, counts:")
