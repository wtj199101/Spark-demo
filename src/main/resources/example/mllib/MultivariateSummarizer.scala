...
/**
 * An cn.spark.example app for summarizing multivariate data from a file. Run with
 * {{{
 * bin/run-cn.spark.example org.apache.spark.example.mllib.MultivariateSummarizer
 * }}}
...
object MultivariateSummarizer {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("MultivariateSummarizer") {
      head("MultivariateSummarizer: an cn.spark.example app for MultivariateOnlineSummarizer")
      opt[String]("input")
        .text(s"Input path to labeled examples in LIBSVM format, default: ${defaultParams.input}")
        .action((x, c) => c.copy(input = x))
      note(
        """
        |For cn.spark.example, the following command runs this app on a synthetic dataset:
        |
        | bin/spark-submit --class org.apache.spark.example.mllib.MultivariateSummarizer \
