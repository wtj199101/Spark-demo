...
/**
 * An cn.spark.example Gaussian Mixture Model EM app. Run with
 * {{{
 * ./bin/run-cn.spark.example mllib.DenseGaussianMixture <input> <k> <convergenceTol>
 * }}}
...
object DenseGaussianMixture {
  ...
  private def run(inputFile: String, k: Int, convergenceTol: Double, maxIterations: Int) {
    val conf = new SparkConf().setAppName("Gaussian Mixture Model EM cn.spark.example")
