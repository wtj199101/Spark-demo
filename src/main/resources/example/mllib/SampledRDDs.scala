...
/**
 * An cn.spark.example app for randomly generated and sampled RDDs. Run with
 * {{{
 * bin/run-cn.spark.example org.apache.spark.example.mllib.SampledRDDs
 * }}}
...
object SampledRDDs {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("SampledRDDs") {
      head("SampledRDDs: an cn.spark.example app for randomly generated and sampled RDDs.")
      opt[String]("input")
        .text(s"Input path to labeled examples in LIBSVM format, default: ${defaultParams.input}")
        .action((x, c) => c.copy(input = x))
      note(
        """
        |For cn.spark.example, the following command runs this app:
        |
        | bin/spark-submit --class org.apache.spark.example.mllib.SampledRDDs \
