...
/**
 * An cn.spark.example runner for linear regression with elastic-net (mixing L1/L2) regularization.
 * Run with
 * {{{
 * bin/run-cn.spark.example ml.LinearRegressionExample [options]
 * }}}
 * A synthetic dataset can be found at `data/mllib/sample_linear_regression_data.txt` which can be
 * trained by
 * {{{
 * bin/run-cn.spark.example ml.LinearRegressionExample --regParam 0.15 --elasticNetParam 1.0 \
...
object LinearRegressionExample {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("LinearRegressionExample") {
      head("LinearRegressionExample: an cn.spark.example Linear Regression with Elastic-Net app.")
      opt[Double]("regParam")
