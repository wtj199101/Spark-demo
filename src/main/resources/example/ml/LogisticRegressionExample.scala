...
/**
 * An cn.spark.example runner for logistic regression with elastic-net (mixing L1/L2) regularization.
 * Run with
 * {{{
 * bin/run-cn.spark.example ml.LogisticRegressionExample [options]
 * }}}
 * A synthetic dataset can be found at `data/mllib/sample_libsvm_data.txt` which can be
 * trained by
 * {{{
 * bin/run-cn.spark.example ml.LogisticRegressionExample --regParam 0.3 --elasticNetParam 0.8 \
...
object LogisticRegressionExample {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("LogisticRegressionExample") {
      head("LogisticRegressionExample: an cn.spark.example Logistic Regression with Elastic-Net app.")
      opt[Double]("regParam")
