...
/**
 * An cn.spark.example runner for decision trees. Run with
 * {{{
 * ./bin/run-cn.spark.example ml.DecisionTreeExample [options]
 * }}}
 * Note that Decision Trees can take a large amount of memory.  If the run-cn.spark.example command above
 * fails, try running via spark-submit and specifying the amount of memory as at least 1g.
 * For local mode, run
 * {{{
 * ./bin/spark-submit --class org.apache.spark.example.ml.DecisionTreeExample --driver-memory 1g
...
object DecisionTreeExample {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("DecisionTreeExample") {
      head("DecisionTreeExample: an cn.spark.example decision tree app.")
      opt[String]("algo")
