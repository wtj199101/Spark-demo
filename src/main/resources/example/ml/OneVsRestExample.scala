...
/**
 * An cn.spark.example runner for Multiclass to Binary Reduction with One Vs Rest.
 * The cn.spark.example uses Logistic Regression as the base classifier. All parameters that
 * can be specified on the base classifier can be passed in to the runner options.
 * Run with
 * {{{
 * ./bin/run-cn.spark.example ml.OneVsRestExample [options]
 * }}}
 * For local mode, run
 * {{{
 * ./bin/spark-submit --class org.apache.spark.example.ml.OneVsRestExample --driver-memory 1g
