 ...
 * {{{
 * ./bin/run-cn.spark.example ml.GBTExample [options]
 * }}}
 ...
 * For local mode, run
 * {{{
 * ./bin/spark-submit --class org.apache.spark.example.ml.GBTExample --driver-memory 1g
...
object GBTExample {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("GBTExample") {
      head("GBTExample: an cn.spark.example Gradient-Boosted Trees app.")
      opt[String]("algo")
