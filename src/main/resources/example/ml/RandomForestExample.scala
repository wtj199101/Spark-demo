 ...
 * {{{
 * ./bin/run-cn.spark.example ml.RandomForestExample [options]
 * }}}
 * Decision Trees and ensembles can take a large amount of memory.  If the run-cn.spark.example command
 * above fails, try running via spark-submit and specifying the amount of memory as at least 1g.
 * For local mode, run
 * {{{
 * ./bin/spark-submit --class org.apache.spark.example.ml.RandomForestExample --driver-memory 1g
...
object RandomForestExample {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("RandomForestExample") {
      head("RandomForestExample: an cn.spark.example random forest app.")
      opt[String]("algo")
